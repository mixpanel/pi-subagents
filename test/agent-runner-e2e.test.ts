...
const REQUIRED_FIXTURE = resolve(fileURLToPath(new URL("./fixtures/e2e-required-ext.mjs", import.meta.url)));
/** The optional fixture registers exactly this tool. */
...
describe("agent-runner end-to-end (real pi-mono session + real extension)", () => {
  ...
  afterEach(() => {
    ...
    delete process.env.PI_SUBAGENT_REQUIRED_BIND_FAILURE;
  ...
  it("loads global required handlers into isolated agents without exposing their tools", async () => {
    ...
    writeFileSync(
      ...
      JSON.stringify({ requiredExtensions: [REQUIRED_FIXTURE] }),
    );
  ...
  it("fails closed when a globally required extension fails to bind", async () => {
    process.env.PI_SUBAGENT_REQUIRED_BIND_FAILURE = "1";
    writeFileSync(
      join(process.env.PI_CODING_AGENT_DIR!, "subagents.json"),
      JSON.stringify({ requiredExtensions: [REQUIRED_FIXTURE] }),
    );
    await expect(activeToolsFor({ extensions: false }, true, true)).rejects.toThrow("Required subagent extension failed to bind");
  });

  it("fails closed when a required extension registers a tool", async () => {
    writeFileSync(
      join(process.env.PI_CODING_AGENT_DIR!, "subagents.json"),
      JSON.stringify({ requiredExtensions: [FIXTURE] }),
    );
    await expect(activeToolsFor({ extensions: false }, true, true)).rejects.toThrow(
      "Required subagent extensions must be handler-only; registered tools: e2e_probe",
    );
  });

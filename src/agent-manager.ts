  ...
  private launch(id: string, record: AgentRecord, args: SpawnArgs, queuedPool: Pool | undefined): Promise<void> {
    const startup = this.startAgent(id, record, args).then(
      ...
      (err) => {
        ...
        } else if (record.worktreeResult?.error) {
          // A stopped-during-copy cleanup failure may be the only pointer to
          // recoverable work. Keep and report that record instead of deleting it.
          this.onComplete?.(record);
        } else {
